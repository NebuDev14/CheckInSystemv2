  ...
  return (
    <>
      {isBus ? (
        ...
        <article className="h-full  col-span-2 flex-col shadow-lg p-2 rounded-2xl ferry-loop duration-200 text-white">
          <div className="h-full flex p-6 pr-0 pb-0 rounded-t-2xl">
            <div className="flex h-full flex-col justify-center">
              <MdDirectionsBoat size={65} />
              <header className="flex flex-col pb-6 mt-auto items-start justify-center leading-tight w-full rounded-b-2xl ">
                <div className="text-3xl  w-full text-left flex flex-col items-start justify-center">
                  {[...Array(2)].map((_, i) => (
                    <div
                      className={`flex flex-col px-4 py-2 w-full rounded-lg my-2  ${
                        ferryData?.at(i + 1).destination.includes("Wall")
                          ? "bg-blue-600"
                          : "bg-red-500"
                      }`}
                      key={i}
                    >
                      <h1 className="text-[2.5rem] py-[0.35rem] font-semibold">
                        {new Date(
                          new Date().getTime() + ferryMinutes?.at(i + 1) * 60000
                        ).toLocaleTimeString("en-US", {
                          timeStyle: "short",
                      ...
                      <h1 className={`mr-auto rounded-lg `}>
                        {ferryData?.at(i + 1).destination}
                      </h1>
                    </div>
                  ))}
            ...
            <div className=" ml-auto px-7 flex flex-col justify-start items-end ">
              <h1 className="text-9xl font-bold">
                {ferryMinutes[0] > 59
                  ? (ferryMinutes[0] / 60).toFixed(0)
                  : ferryMinutes[0]}
              ...
              <div className="pt-2 mb-4">
                <span
                  className={`ml-6 px-3 text-3xl rounded-lg ${
                    ferryData?.at(0).destination.includes("Wall")
                      ? "bg-blue-600"
                      : "bg-red-500"
                  } py-2`}
                >
