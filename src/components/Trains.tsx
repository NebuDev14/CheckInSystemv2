  ...
  return (
    <div className="grid grid-cols-2 gap-6 mb-5 mt-2 ">
      <article className="h-full flex flex-col  shadow-lg rounded-2xl bg-zinc-100 border-zinc-800">
        ...
        <header
          className={`flex flex-col items-left  h-full  pl-4 pr-2 leading-tight w-full ${
            queens.length === 0 && manhattan.length === 0 ? " py-72" : "py-3"
          }`}
        >
          ...
          {queens.slice(1, 3).map((train, i) => (
            <div className="grid grid-cols-8 mb-3 ml-2" key={i}>
              ...
              <h1 className="mr-6 col-span-2 text-5xl font-semibold flex items-end justify-end">
                {(
                  (train.time.getTime() - new Date().getTime()) /
                  1000 /
                  60
                ).toFixed(0)}
      ...
      <article className="h-full flex flex-col  shadow-lg rounded-2xl bg-zinc-100 border-zinc-800 min-h-[860px]">
        ...
        <header className="flex flex-col items-left  h-full py-3 pl-4 pr-2 leading-tight w-full">
          <div className="flex items-center justify-center flex-col mb-12">
          ...
          {manhattan.slice(1, 3).map((train, i) => (
            <div className="grid grid-cols-8 mb-3 ml-2" key={i}>
              ...
              <h1 className="-ml-6 col-span-5  text-3xl mr-4 text-left flex items-center justify-center">
                {train.destination.length > 17
                  ? train.destination.slice(0, 14) + "..."
                  : train.destination}
              </h1>
              <h1 className="mr-6 col-span-2 text-5xl font-semibold flex items-end justify-end">
                {(
